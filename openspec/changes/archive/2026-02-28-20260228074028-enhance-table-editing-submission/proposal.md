...
当前 `@maita-table` 已实现基础的单元格编辑能力（参考变更 `add-maita-table-editing`），但在企业级场景下仍存在验证能力不够结构化、提交模式单一、编辑状态耦合导致难以测试/复用，以及大数据量下频繁状态更新带来的性能风险等问题。本变更旨在在**保持向后兼容**的前提下补齐这些能力缺口。

## 变更内容

- **结构化验证**：引入 Zod Schema 作为列级/行级验证能力，并与旧 `validate` 函数向后兼容
- **灵活提交模式**：支持即时提交（默认）、单行提交、批量提交三种编辑提交模式
- **状态解耦**：将编辑状态与 UI 组件解耦，提升可测试性与可维护性
- **性能优化**：引入防抖/节流与批量更新策略以适配大数据量场景
