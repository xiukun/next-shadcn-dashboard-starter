...
## 3. 阶段 2：浮动过滤与基础过滤
- [ ] 3.1 创建 `packages/maita-table-react/src/components/FilterIndicator.tsx`，显示过滤状态
- [ ] 3.2 创建 `packages/maita-table-react/src/components/FloatingFilter.tsx`，实现浮动过滤器
- [ ] 3.3 在浮动过滤器中集成防抖处理（使用现有的 `useDebouncedCallback`）
...
- [ ] 3.9 创建 `packages/maita-table-react/src/hooks/useColumnFiltering.ts`，集成 TanStack Table 的过滤功能
- [ ] 3.10 集成 TanStack Table 的 `getFilteredRowModel`，支持前端过滤
- [ ] 3.11 为过滤功能添加单元测试
